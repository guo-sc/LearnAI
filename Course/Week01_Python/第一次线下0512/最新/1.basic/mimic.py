...
def mimic_dict(filename):
    ...
    with open(filename) as f:
        ...
        Str =Str.lower()
        Str_list = Str.split()
        Str_dict = {" ":Str_list,Str_list[-1]:[" "]}
        
        for i in range(len(Str_list)-1):
            Str_dict[Str_list[i]] = Str_dict.get(Str_list[i],[])
            Str_dict[Str_list[i]].append(Str_list[i+1])
...
def print_mimic(mimic_dict, word):
    ...
    words = []
    for i in range(200):
        words.append(random.choice(mimic_dict[word]))
        word = words[-1]
